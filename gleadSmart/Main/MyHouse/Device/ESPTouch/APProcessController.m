...
- (void)bindDevice:(DeviceModel *)device success:(void(^)(void))success failure:(void(^)(void))failure{
    ...
    if (homeList.count <= 0) {
        ...
        parameters = @{@"type":postType,@"mac":device.mac,@"name":device.name,@"roomUid":db.currentHouse.houseUid,@"houseUid":db.currentHouse.houseUid,@"companyId":@"000001"};
    }else{
        RoomModel *room = homeList[0];//将新设备插入到家庭第一个房间
        parameters = @{@"type":postType,@"mac":device.mac,@"name":device.name,@"roomUid":room.roomUid,@"houseUid":db.currentHouse.houseUid,@"companyId":@"000001"};
    }
    NSLog(@"%@",parameters);
